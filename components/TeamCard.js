...
class TeamCard extends Component {
  ...
  render() {
   ...
   if (this.props.imgUrl !== '') {
     cardImg = `<img src="${this.props.imgUrl}" title="${this.props.name}" />`;
   } else {
      ...
      <div className="card horizontal hoverable team-card" data-slug={this.props.slug}>
       ...
       <div id={this.props.slug} className="modal modal-fixed-footer" style={{cursor: 'auto'}}>
         <div className="modal-content">
           <h4 style={{color: 'rgba(0, 0, 0, 0.87)'}} dangerouslySetInnerHTML={{__html: this.props.title}}></h4>
           <p dangerouslySetInnerHTML={{__html: this.props.content}}>

           </p>
         </div>
         <div className="modal-footer">
           <a className="waves-effect btn-flat" data-slug={this.props.slug}>Close</a>
         </div>
        </div>
      </div>
    );
