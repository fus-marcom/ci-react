...
export default class extends React.Component {
  ...
  static async getInitialProps () {
    ...
    const params = 'resource?per_page=100&fields=title,acf,better_featured_image';
      ...
      <Masonry>
        {this.props.data.filter(post => activeTab === 'all' || activeTab === post.acf.type).map( (post, i) => (
          <div className="col s12 m6 l4 xl3" key={i}>
            <ResourceCard title={post.title.rendered} type={post.acf.type} content={post.acf.description} url={post.acf.url} price={post.acf.price} img={post.better_featured_image !== null ? post.better_featured_image.source_url : ''} imgWidth={post.better_featured_image !== null ? post.better_featured_image.media_details.width : '1'} />
          </div>
        ))
        }
      </Masonry>
